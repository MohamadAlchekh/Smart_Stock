                ...
                builder: (context, statistics, child) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      final log = statistics.logs[statistics.logs.length - 1 - index];
                      return Card(
...
class _SettingsPageState extends State<SettingsPage> {
      ...
      child: ListView(
        ...
        children: [
          ...
          _buildSettingsSection(
            ...
            [
              _buildSettingsTile(context, 'settings_page.sync'.tr(), Icons.sync),
              _buildSettingsTile(context, 'settings_page.backup'.tr(), Icons.backup),
              _buildSettingsTile(context, 'settings_page.auto_update'.tr(), Icons.system_update),
            ],
          ),
          const SizedBox(height: 16),
          _buildSettingsSection(
            ...
            [
              _buildSettingsTile(context, 'settings_page.app_version'.tr(), Icons.info),
              _buildSettingsTile(context, 'settings_page.privacy_policy'.tr(), Icons.privacy_tip),
              _buildSettingsTile(context, 'settings_page.terms_of_use'.tr(), Icons.description),
            ],
          ),
        ],
      ),
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildThemeOption(context, 'theme_dialog.blue_theme'.tr(), 'blue', Icons.circle, const Color(0xFF1A237E)),
            _buildThemeOption(context, 'theme_dialog.dark_theme'.tr(), 'dark', Icons.circle, Colors.black),
          ],
        ),
        actions: [
          TextButton(
  ...
  Widget _buildThemeOption(BuildContext context, String title, String themeKey, IconData icon, Color color) {
    final isRTL = context.locale.languageCode == 'ar';
    final isDarkMode = Theme.of(context).brightness == Brightness.dark;
    
    return ListTile(
      leading: isRTL ? null : Icon(icon, color: isDarkMode ? Colors.white : color),
      trailing: isRTL ? Icon(icon, color: isDarkMode ? Colors.white : color) : null,
      title: Text(
        title,
        textAlign: isRTL ? TextAlign.right : TextAlign.left,
        style: TextStyle(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildLanguageOption(context, 'Türkçe', 'tr', Icons.language, '🇹🇷'),
            _buildLanguageOption(context, 'English', 'en', Icons.language, '🇬🇧'),
  ...
  Widget _buildLanguageOption(BuildContext context, String title, String langCode, IconData icon, String flag) {
    final isRTL = context.locale.languageCode == 'ar';
    final isDarkMode = Theme.of(context).brightness == Brightness.dark;
...
class _WarehousesPageState extends State<WarehousesPage> {
  static List<Map<String, dynamic>>? _warehouses;
      ...
      _warehouses = [
        {
          ...
          'blocks': List<Map<String, dynamic>>.generate(6, (index) => {
            'name': 'Blok ${index + 1}',
            'products': '0',
            'products_list': [],
          }),
  ...
  void _addWarehouse(String name, IconData icon, int blockCount) {
    setState(() {
      _warehouses!.add({
        ...
        'products': '0',
        'icon': icon,
        'blocks': List<Map<String, dynamic>>.generate(blockCount, (index) => {
          'name': 'Blok ${index + 1}',
          'products': '0',
          'products_list': [],
        }),
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () {
                final name = nameController.text.trim();
                final blockCount = int.tryParse(blockCountController.text.trim()) ?? 0;
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return _buildWarehouseCard(
              ...
              'total_products'.tr(args: [warehouse['products']]),
              warehouse['icon'] as IconData,
              List<Map<String, dynamic>>.from(warehouse['blocks'] as List),
              warehouse['isExpanded'] as bool,
              () => _toggleWarehouse(index),
            );
          },
        ),
      ),
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => WarehouseDetailScreen(
                title: title,
                products: blocks.fold<int>(0, (sum, block) => sum + int.parse(block['products'] ?? '0')).toString(),
                icon: icon,
                blocks: blocks,
                isAdmin: widget.isAdmin,
                userName: widget.userName,
                ...
                onBlocksUpdated: (updatedBlocks) {
                  setState(() {
                    final warehouseIndex = _warehouses!.indexWhere((w) => w['name'] == title);
                    if (warehouseIndex != -1) {
                      _warehouses![warehouseIndex]['blocks'] = updatedBlocks;
                      // Toplam ürün sayısını güncelle
                      final totalProducts = updatedBlocks.fold<int>(
                        0,
                        (sum, block) => sum + int.parse(block['products'] ?? '0'),
                      );
                      _warehouses![warehouseIndex]['products'] = totalProducts.toString();
                    }
                  });
                },
              ),
  ...
  void _toggleWarehouse(int index) {
    setState(() {
      _warehouses![index]['isExpanded'] = !_warehouses![index]['isExpanded'];
    });
  }
}
